...
class Checkout extends Component<CheckoutProps & WithCheckoutProps & WithLanguageProps, CheckoutState> {
    ...
    async componentDidMount(): Promise<void> {
        ...
        try {
            ...
            const getApiData = async () => {
                const res: any = await fetch("http://localhost:5000/api/address/search?query=vintage", {
                    method: "get",
                    headers: {
                        "access-token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MDllZTI2NWNmYTAyMWU5MzkxOTE1NDgiLCJmaXJzdE5hbWUiOiJ0ZXN0IiwibGFzdE5hbWUiOiJ0ZXN0IiwiZW1haWwiOiJjb3JleWtvZ2FuQGdtYWlsLmNvbSIsImlhdCI6MTYyMTUyODk4MSwiZXhwIjoxNjIxNTM2MTgxfQ.frXK55-L1KTObM6jA_h-L1t25PKkBvF4DWe7exk93nc"
                    }
                })
            ...
            getApiData().then(response => response.json()).then(data => this.setState({
                dealerLocations: data.data
            }))
    ...
    private renderStep(step: CheckoutStepStatus): ReactNode {
        ...
        case CheckoutStepType.FLLStep:
            return this.renderFLLStep(step);
    ...
    private renderFLLStep(step: CheckoutStepStatus): ReactNode {
        const {
            cart,
        } = this.props;

        console.log(this.state.dealerLocations)
        ...
        return (
            <CheckoutStep
                { ...step }
                heading={ <TranslatedString id="ffl.ffl_heading" /> }
                key={ step.type }
